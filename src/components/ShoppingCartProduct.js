...
import styles from "./../css/ShoppingCart/ShoppingCartProduct.module.css";
...
const ShoppingCartProduct = (props) => {
  ...
  return (
    <div className={styles.ShoppingCartProduct}>
      ...
      <div className={styles.Productqty}>{props.qty}x</div>
      <button className={styles.MinusButton} onClick={() => props.DecreaseAmount(props.id, props.cost)}>
        -
      </button>
      <div className={styles.ProductName}>{props.value}</div>
      <div className={styles.ProductCost}>{CombinedCost} €</div>
